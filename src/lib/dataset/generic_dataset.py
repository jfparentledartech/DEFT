...
class GenericDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if opt.tracking:
            ...
            pre_img, pre_img_org = self._get_input(pre_image, trans_input_pre)
        ...
        if opt.AFE:
            ...
            pre_image_AFE, pre_image_AFE_org = self._get_input(
                pre_image_AFE, trans_input_pre_AFE
            )
            bboxes_pre, track_ids_pre = self._get_data_AFE(
                pre_anns_AFE, trans_input_pre_AFE, trans_output_pre_AFE
            )
            ...
            if len(bboxes_pre) > 0:
                ...
                bboxes_pre = np.empty([0, 4])
            if len(bboxes_next) > 0:
                bboxes_next = np.array(bboxes_next)
            else:
                bboxes_next = np.empty([0, 4])

            track_ids_pre = np.array(track_ids_pre)
            track_ids_next = np.array(track_ids_next)
            labels = np.repeat(
                np.expand_dims(np.array(track_ids_pre), axis=1),
                len(track_ids_next),
                axis=1,
            ) == np.repeat(
                np.expand_dims(np.array(track_ids_next), axis=0),
                len(track_ids_pre),
                axis=0,
            )
            labels = np.pad(
                labels,
                [
                    (0, max(0, self.max_object - labels.shape[0])),
                    (0, max(0, self.max_object - labels.shape[1])),
                ],
                mode="constant",
                constant_values=0,
            )

            #       im = show_matching_hanlded_rectangle(pre_image_AFE_org, inp_org, bboxes_pre, bboxes_next, labels)
            #       cv2.imwrite( '/s/red/a/nobackup/vision/chaabane/tracker/DEFT/src/matching.png', im)

            bboxes_pre, bboxes_next = ToPercentCoordinates(
                bboxes_pre, bboxes_next, inp_org
            )
            bboxes_pre, bboxes_next, labels = ResizeShuffleBoxes(
                max_object=self.max_object,
                boxes_pre=bboxes_pre,
                boxes_next=bboxes_next,
                labels=labels,
            )

            bboxes_pre, bboxes_next, labels = FormatBoxes(
                boxes_pre=bboxes_pre, boxes_next=bboxes_next, labels=labels
            )

            bboxes_pre, current_indexes, bboxes_next, next_indexes, labels = ToTensor(
                boxes_pre=bboxes_pre, boxes_next=bboxes_next, labels=labels
            )

            ret["bboxes_pre"] = bboxes_pre.clone()
            ret["bboxes_next"] = bboxes_next.clone()

            ret["current_indexes"] = current_indexes
            ret["next_indexes"] = next_indexes
            ret["labels"] = labels
    ...
    def _load_pre_data_AFE(self, video_id, frame_id, sensor_id=1):
        ...
        if "train" in self.split:
            img_ids = [
                ...
                and abs(img_info["frame_id"] - frame_id) > 0
                and (
    ...
    def _load_pre_data(self, video_id, frame_id, sensor_id=1):
        ...
        if "train" in self.split:
            img_ids = [
                ...
                and img_info["frame_id"] - frame_id < 0
                and (
                    not ("sensor_id" in img_info) or img_info["sensor_id"] == sensor_id
                )
            ]
