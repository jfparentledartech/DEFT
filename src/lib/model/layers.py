...
class Compressor(nn.Module):
    """ Encoding module.  
        ...
        Operations:
            Batch Normalization -> Convolution ->  Activation -> Pooling

        The output is shrinked by a factor given as an argument

        Args:
            in_channels - number of channel features in
            out_channels - number of channel features out
            kernel - convolution kernel size, 2D or 3D.
            padding - if True, preserves the shape of the input
            activation - activation function (typically nn.ReLU())
            shrink - downsampling factor along 2 axes (height, width) or 3 axes (depth, height, width)
            pooling - 'max' or 'average'
    """
    def __init__(self, in_channels:int, out_channels:int, kernel:tuple=(3,3,3), padding:bool=True, 
                 activation=nn.ReLU(), shrink:tuple=(1,1,1), pooling:str='max'):
        ...
        if len(kernel) == 2:
            self.bn = nn.BatchNorm2d(in_channels)
            self.conv = nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=kernel, padding=padding) 
            self.activation = activation
            if pooling == 'max':
                self.pool = nn.MaxPool2d(kernel_size=shrink, stride=shrink)
            elif pooling == 'average':
                self.pool = nn.AvgPool2d(kernel_size=shrink, stride=shrink)

        if len(kernel) == 3:
            self.bn = nn.BatchNorm3d(in_channels)
            self.conv = nn.Conv3d(in_channels=in_channels, out_channels=out_channels, kernel_size=kernel, padding=padding) 
            self.activation = activation
            if pooling == 'max':
                self.pool = nn.MaxPool3d(kernel_size=shrink, stride=shrink)
            elif pooling == 'average':
                self.pool = nn.AvgPool3d(kernel_size=shrink, stride=shrink)
...
class Upscale(nn.Module):
    ...
    """
    def __init__(self, in_channels:int, out_channels:int, kernel:tuple=(2,1,1), up_factor:tuple=(2,1,1), activation=None):
        ...
        if len(kernel) == 2: #(2d)
            ...
            self.activation = activation
            self.bn = nn.BatchNorm3d(in_channels)
            self.trans = nn.ConvTranspose3d(in_channels=in_channels, out_channels=out_channels, kernel_size=kernel, 
...
class Decapitation(nn.Module):
    """ Extracts 2D feature maps from 3D input data and removes depth dimension.

        Operations:
        Batch norm -> Convolution -> Activation -> Pooling (entire depth channel) -> remove depth dim
        ...
        Args:
            ...
            pooling - 'max' or 'average' 
    """
    def __init__(self, in_channels:int, out_channels:int, kernel:tuple=(3,3,3), activation=nn.ReLU()):
        super(Decapitation, self).__init__()

        self.bn = nn.BatchNorm3d(in_channels)
        self.conv = nn.Conv3d(in_channels=in_channels, out_channels=out_channels, kernel_size=kernel, 
                              padding=(int((kernel[0]-1)/2),int((kernel[1]-1)/2),int((kernel[2]-1)/2)))
        self.activation = activation
        self.out_channels = out_channels
