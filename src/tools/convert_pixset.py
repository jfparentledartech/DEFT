...
if __name__ == '__main__':
    ...
    for i_frame, frame in enumerate(range(len(sc))):
        ...
        for i_detection, detection in enumerate(annotation_sample.raw['data']):
            ...
            v = np.dot(annotation_to_camera_transformation[:3,:3], np.array([1, 0, 0]))
            ...
            amodel_center = project_pts(np.asarray([center_coordinates]), camera_intrinsic).tolist()
            # amodel_center = project_to_image(
            #     np.array(
            #         [
            #             center_coordinates[0],
            #             center_coordinates[1] - box_dimensions[0] / 2,
